...
export default function PrintLeaveForm() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const detailsRes = await fetch(`${API_BASE_URL}/leave/details/${id}`, {
          headers: getHeaders(),
        });
  ...
  // approved before this page loads). "Total Earned" is reconstructed as
  // current balance + this application's days, ONLY for whichever leave type
  // (VL or SL) this application actually drew from. This is an approximation
  // for display purposes, not a new source of truth — the database balance
    ...
    <div className="min-h-screen bg-muted/30 py-8 print:bg-white print:py-0">
      ...
      <div className="max-w-3xl mx-auto mb-4 flex justify-between items-center print:hidden">
        <p className="text-sm text-muted-foreground">
          Application {application.application_number} — ready to print
        </p>
      ...
      {/* Printable form */}
      <div className="max-w-3xl mx-auto bg-white text-black p-8 shadow print:shadow-none print:p-0 text-sm">
